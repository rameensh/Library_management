...
//   }
// };
// catalogue.js - Replace the quickBorrow function

// ─── Quick Borrow Function (FIXED) ──────────────────────────────────
...
window.quickBorrow = async function(bookId, bookTitle) {
  ...
  // Confirm with user
  if (!confirm(`Borrow "${bookTitle}"?\n\nYou'll have 14 days to return it.`)) {
    return;
  }
  ...
  try {
    ...
    const endpoint = `${API_BASE_URL}/api/borrow`;
    ...
    if (response.status === 401) {
      // Token expired
      localStorage.removeItem('bibliotheca:token');
      localStorage.removeItem('bibliotheca:user');
    ...
    if (response.status === 400) {
      alert(data.detail || 'You can only borrow one book at a time.');
      if (btn) {
        btn.disabled = false;
    ...
    if (!response.ok) {
      throw new Error(data.detail || data.message || 'Failed to borrow book');
    }
    
    // Success!
    alert(`✅ "${bookTitle}" borrowed successfully!\n\nDue date: ${new Date(data.due_date || Date.now() + 14*24*60*60*1000).toLocaleDateString()}`);
